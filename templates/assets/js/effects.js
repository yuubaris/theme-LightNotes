    !function() {
        ...
        const requestAnimationFrame = raf || function(callback) {
            ...
            const timeToCall = Math.max(0, 16 - (currTime - lastTimestamp));
            const id = setTimeout(function() {
                callback(currTime + timeToCall);
            }, timeToCall);
            lastTimestamp = currTime + timeToCall;
            return id;
        };
        
        const cancelAnimationFrame = cancelRaf || function(id) {
            clearTimeout(id);
        };
        ...
        function SakuraEffect() {
            ...
            // 樱花花瓣对象
            function Sakura(x, y, scale, rotate, fn) {
                this.x = x;
                this.y = y;
                this.s = scale;
                this.r = rotate;
            ...
            Sakura.prototype.initSpeed = function() {
                // 固定速度参数，避免动态加载后速度异常
                this.speed.x = (Math.random() - 0.5) * 0.8;
                this.speed.y = 1.5 + Math.random() * 0.5;
                this.speed.r = 0.01 + Math.random() * 0.02;
            };
            ...
            Sakura.prototype.draw = function(ctx) {
                ...
                ctx.rotate(this.r);
                ctx.globalAlpha = 0.7;
                ctx.drawImage(img, -20 * this.s, -20 * this.s, 40 * this.s, 40 * this.s);
                ctx.restore();
            };
            
            Sakura.prototype.update = function(canvasWidth, canvasHeight) {
                // 使用固定的增量，不受帧率影响
                this.x += this.speed.x;
                this.y += this.speed.y;
                this.r += this.speed.r;
            ...
            // 创建樱花
            function createSakura(canvasWidth, canvasHeight) {
                const x = Math.random() * canvasWidth;
                const y = Math.random() * canvasHeight;
                const s = 0.5 + Math.random() * 0.5;
                const r = Math.random() * Math.PI * 2;
                
                const sakura = new Sakura(x, y, s, r, null);
                return sakura;
            }
            
            // 初始化樱花列表
            function initSakuraList(canvasWidth, canvasHeight) {
                sakuraList = [];
                const count = IS_MOBILE ? 15 : 30;
                
                for (let i = 0; i < count; i++) {
                    sakuraList.push(createSakura(canvasWidth, canvasHeight));
                }
            }
            
            // 公开方法
            this.start = async function(canvas, ctx) {
                await loadImage();
                initSakuraList(canvas.width, canvas.height);
            };
            ...
            this.update = function(canvas, ctx) {
                ...
                for (let i = 0; i < sakuraList.length; i++) {
                    sakuraList[i].update(canvas.width, canvas.height);
                    sakuraList[i].draw(ctx);
                }
            };
        ...
        function SnowEffect() {
            let flakes = [];
            let lastUpdate = 0;
            const fixedDelta = 16; // 60fps对应的每帧时间
            
            // 雪花类
            class Snowflake {
                constructor(canvasWidth, canvasHeight, maxSize, fallSpeed) {
                    ...
                    this.velX = 0;
                    this.stepSize = Math.random() / 50;
                    this.step = Math.random() * Math.PI * 2;
                    this.wind = (Math.random() - 0.5) * 0.3;
                }
                
                update(canvasWidth, canvasHeight, delta) {
                    // 使用时间增量确保速度稳定
                    const timeFactor = delta / 16;
                    
                    this.velX = this.wind + Math.cos(this.step) * this.stepSize;
                    this.step += 0.05;
                    
                    this.y += this.velY * timeFactor;
                    this.x += this.velX * timeFactor;
                ...
                reset(canvasWidth, canvasHeight) {
                    ...
                    this.speed = Math.random() * 0.5 + this.fallSpeed;
                    this.velY = this.speed;
                    this.wind = (Math.random() - 0.5) * 0.3;
                }
            ...
            // 初始化雪花
            function initSnowflakes(canvasWidth, canvasHeight) {
                flakes = [];
                const maxFlakes = IS_MOBILE ? 25 : 60;
                const flakeSize = 8;
                const fallSpeed = 1.5;
                
                for (let i = 0; i < maxFlakes; i++) {
                    flakes.push(new Snowflake(canvasWidth, canvasHeight, flakeSize, fallSpeed));
                }
                lastUpdate = performance.now();
            }
            
            // 公开方法
            this.start = function(canvas, ctx) {
                initSnowflakes(canvas.width, canvas.height);
            };
            
            this.update = function(canvas, ctx) {
                const now = performance.now();
                const delta = lastUpdate ? Math.min(now - lastUpdate, 32) : 16;
                lastUpdate = now;
                
                ctx.clearRect(0, 0, canvas.width, canvas.height);
                
                for (let i = 0; i < flakes.length; i++) {
                    flakes[i].update(canvas.width, canvas.height, delta);
                    flakes[i].render(ctx);
                }
            };
        ...
        function createCanvas() {
            const canvas = document.createElement("canvas");
            canvas.id = "theme_effects";
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            canvas.setAttribute("class", "canvas_effects");
            canvas.setAttribute(
                "style",
        ...
        function animate(timestamp) {
            if (!currentCanvas || !currentEffect) {
                return;
            }
            
            // 限制帧率，防止速度异常
            frameCount++;
            if (frameCount % 2 !== 0) { // 大约30fps
            ...
            try {
                ...
                if (ctx && currentEffect.update) {
                    currentEffect.update(currentCanvas, ctx);
                }
            } catch (e) {
                console.error("Effect animation error:", e);
                clearEffect();
            }
        ...
        async function startEffect(effectType) {
            ...
            if (effectType === 'flower') {
                currentEffect = new SakuraEffect();
            ...
            if (currentEffect.start) {
                await currentEffect.start(canvas, ctx);
            }
        ...
        // ---------- 获取当前主题（适配晴绫主题 data-color-scheme） ----------
        function getCurrentTheme() {
            var scheme = document.documentElement.getAttribute('data-color-scheme');
            if (scheme === 'dark') return 'snow';
            if (scheme === 'light') return 'flower';
            // 兜底：检查 localStorage
            try {
                var saved = localStorage.getItem('ql-theme');
                if (saved === 'dark') return 'snow';
                if (saved === 'light') return 'flower';
            } catch (e) {}
            return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'snow' : 'flower';
        }
        
        // ---------- 解析期望特效：优先使用主题配置中用户选择的类型 ----------
        function getDesiredEffect() {
            var cfg = window.lnEffects || {};
            var type = cfg.type || 'auto';
            if (type === 'flower' || type === 'snow') return type;
            // auto：跟随明暗模式（浅色樱花、深色雪花）
        ...
        function init() {
            ...
            // 初始启动
            startEffect(getDesiredEffect());
            
            // 监听窗口大小变化
            window.addEventListener('resize', handleResize);
            
            // 监听 data-color-scheme 属性变化（与主题明暗模式同步）
            const schemeObserver = new MutationObserver(function() {
                setTimeout(() => startEffect(getDesiredEffect()), 100);
            });
            ...
            const domObserver = new MutationObserver(function(mutations) {
                ...
                if (currentCanvas && !document.body.contains(currentCanvas)) {
                    clearEffect();
                    setTimeout(() => startEffect(getDesiredEffect()), 500);
                }
            });
